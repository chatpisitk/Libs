...
public class CustomViewAbove extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		/*switch(getContext().getResources().getConfiguration().orientation){
		  case Configuration.ORIENTATION_LANDSCAPE:		  
		    mContent.measure(contentWidth - getChildLeft(1), contentHeight);	  
		  break;		 
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		mContent.layout(contentLeft, 0, width + contentLeft, height);
		
		
		/*switch(getContext().getResources().getConfiguration().orientation){
		  case Configuration.ORIENTATION_LANDSCAPE:		  
			if(mCurItem == 0)
		      mContent.layout(contentLeft, 0, width, height);
			else
			  mContent.layout(contentLeft, 0, width + contentLeft, height);	
		  break;		 
		  case Configuration.ORIENTATION_PORTRAIT:
